...
def settings_cmd():
    ...
    """
    if len(sys.argv) == 1:  # if no parameters in the cmd, the program asks user for them in the console
        rotors_settings = input("Enter states for three rotors (format: AAA/БАА...): ")
        conn_panel = input("Enter settings for the connecting panel (format: AБ/БВ...): ")
    elif len(sys.argv) == 3:  # if there are parameters in the command line
        rotors_settings = str(sys.argv[1])
        conn_panel = str(sys.argv[2])
    else:
         raise ValueError("Only 2 or 3 parameters are allowed in command line!")
...
def my_text_cmd():
    ...
    """
    if len(sys.argv) == 4:
        text_to_encrypt = str(sys.argv[3])
        return text_to_encrypt
    text = input("Enter your text: ").upper()  # initial text
    return text
...
def main():
    global rotors
    encrypted = ''
    rot_sett = settings_cmd()
    my_text = my_text_cmd()
    get_configurations()
    rotors = change_rot_sett(rot_sett)
    for letter in my_text:
        ...
        letter = lett_to_conn(letter)
        encrypted += letter
    print(encrypted)  # resulted text after Enigma
